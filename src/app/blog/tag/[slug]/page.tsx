...
export async function generateMetadata({
  ...
  const title = `「${tag.name}」のコラム`;
  const description = `「${tag.name}」タグが付いた占いコラムの一覧です。`;
  const url = absoluteUrl(`/blog/tag/${tag.slug}`);
  return {
    title,
    description,
...
export default async function BlogTagPage({
  ...
  const query = parsePostParams(raw, { tagSlug: tag.slug });
